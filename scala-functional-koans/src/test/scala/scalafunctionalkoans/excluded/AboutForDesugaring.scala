package scalafunctionalkoans.excluded
...
class AboutForDesugaring extends KoanSuite {
  ...
  koan(
    """Alternatively, elements of range can be filtered
  ...
  koan(
    """It is possible to iterate over two nested ranges
  ...
  koan(
    """Alternatively, it is possible to iterate over two nested ranges
  ...
  koan(
    """IndexedSeq can be used in desugared for comprehension
      | with two generators""") {
